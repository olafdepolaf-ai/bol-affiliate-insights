...
class ScanPage {
	...
	public function render(): void {
		...
		$page_url    = admin_url( 'admin.php?page=affiliate-link-checker' );
		...
		$check_url      = wp_nonce_url(
			add_query_arg( 'alc_check_updates', '1', $page_url . ( $current_tab !== 'scanner' ? '&tab=' . $current_tab : '' ) ),
			'alc_check_updates'
		);
		?>
		<div class="wrap">
	...
	private function render_scanner(): void {
		...
		$nonce = wp_create_nonce( 'alc_run_scan_nonce' );
		?>
		...
		(function() {
			...
			function scanNext(index) {
				...
				data.append('action',   'alc_check_link');
