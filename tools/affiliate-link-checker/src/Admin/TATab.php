...
class TATab {
	...
	private function render_aanbeveling_404_subtab(): void {
		$allowed_periods = \TuinenBalkon\AffiliateLinkChecker\Service\OrphanedLinkScanner::PERIODS;
		...
		$nonce      = wp_create_nonce( 'alc_orphan_nonce' );
		?>
		<style>
		...
		(function() {
			...
			document.querySelectorAll('.alc-find-btn').forEach(function(btn) {
				btn.addEventListener('click', function() {
					...
					data.append('action', 'alc_orphan_find_articles');
	...
	private function render_orphaned_links_subtab(): void {
		...
		(function() {
			...
			function startScan() {
				...
				initData.append('action', 'alc_orphan_init');
			...
			function runBatch(offset, totalPosts, allOrphans) {
				...
				var data = new FormData();
				data.append('action', 'alc_orphan_batch');
				data.append('nonce',  nonce);
			...
			function saveScan(orphans) {
				...
				var data = new FormData();
				data.append('action',  'alc_orphan_save');
				data.append('nonce',   nonce);
