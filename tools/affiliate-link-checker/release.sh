...
PLUGIN_FILE="affiliate-link-checker.php"
BUMP=${1:-patch}
...
# Zip bouwen vanuit de tools/ map
cd ..
rm -f affiliate-link-checker.zip
zip -r affiliate-link-checker.zip affiliate-link-checker/ \
    --exclude "affiliate-link-checker/release.sh" \
    --exclude "*/.DS_Store"
echo "Zip gebouwd: tools/affiliate-link-checker.zip"
cd affiliate-link-checker

# Commit en tag
git -C ../.. add \
    "tools/affiliate-link-checker/$PLUGIN_FILE" \
    "tools/affiliate-link-checker.zip"
git -C ../.. commit -m "chore(alc): bump version to $NEW_VERSION"
git -C ../.. tag "$TAG"
...
gh release create "$TAG" \
    ...
    --title "Affiliate Link Checker v$NEW_VERSION" \
    --notes "Versie $NEW_VERSION" \
    "../affiliate-link-checker.zip#affiliate-link-checker.zip"
