...
/**
 * Adapter for the ThirstyAffiliates plugin.
 *
 * Uses is_plugin_active() as primary availability check and direct $wpdb
 * queries for link retrieval, so it works regardless of post-type registration
 * timing or object-cache state.
 */
class ThirstyAffiliatesAdapter implements AffiliateLinkAdapterInterface {
    ...
    public function is_available(): bool {
        ...
        // Primary: check if the plugin file is active.
        if ( is_plugin_active( 'thirstyaffiliates/thirstyaffiliates.php' ) ) {
            return true;
        }
        // Fallback: check if the post type was registered (works after init).
    ...
    public function get_link_redirect_url( int $link_id ): ?string {
        ...
        // get_permalink works once the post type is registered (during admin page render).
    ...
    public function get_all_links(): array {
        ...
        // Use a direct query so this works even if the post type is not yet
        // registered in the current request lifecycle.
    ...
    public function build_bol_params_index(): array {
        ...
        foreach ( $this->get_all_links() as $link ) {
            $url = $link['url'];
            // Only process bol.com affiliate URLs.
